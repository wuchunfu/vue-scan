(function () {
  ...
  function highlight(instance, uuid, flashCount, options) {
    ...
    if (document.getElementById(uuid)) {
      ...
      const el2 = document.getElementById(uuid);
      if (el2?.style) {
        el2.style.opacity = "1";
      }
      return;
...
`;
  ...
  function findMatchingDomain(host, domainMap) {
  ...
  function initializeObserver(node) {
    let observer;
    const callback = () => {
      if (node.__vue_app__ && window.__VUE_SCAN__) {
        let mixinChildren = function(children) {
          if (!children) {
            return;
          }
        ...
        }, mixin = function(vueInstance2) {
          if (vueInstance2.subTree?.el && vueInstance2.__vue_scan_injected__ !== true) {
            const onBeforeUpdate = createOnBeforeUpdateHook(vueInstance2);
            const onBeforeUnmount = createOnBeforeUnmountHook(vueInstance2);
            if (onBeforeUpdate) {
              if (vueInstance2?.bu) {
                vueInstance2.bu.push(onBeforeUpdate);
              } else {
                vueInstance2.bu = [onBeforeUpdate];
              }
            }
            if (onBeforeUnmount) {
              if (vueInstance2?.bum) {
                vueInstance2.bum.push(onBeforeUnmount);
              } else {
                vueInstance2.bum = [onBeforeUnmount];
              }
            }
          }
          vueInstance2.__vue_scan_injected__ = true;
          if (!vueInstance2?.subTree?.component && vueInstance2?.subTree?.children) {
            mixinChildren(vueInstance2.subTree.children);
          } else if (vueInstance2?.subTree?.component) {
            mixin(vueInstance2.subTree.component);
          } else if (!vueInstance2?.subTree && vueInstance2?.children) {
            mixinChildren(vueInstance2.children);
          }
        };
        observer?.disconnect();
        node.__vue_app__.use(window.__VUE_SCAN__.plugin);
        const vueInstance = node.__vue_app__._container._vnode.component;
